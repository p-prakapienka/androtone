...
public:
    Chorus() {
        // 0.86 Hz approximates the fixed modulation rate of Juno Chorus I.
        rateHz.store(0.86f);
        depthMs.store(4.0f);
        centreDelayMs.store(8.0f);
        feedback.store(0.1f);
        mix.store(0.5f);
        cutoffHz.store(4000.0f);
        filterCoefficient.store(0.0f);
    }
    ...
    void prepare(double newSampleRate) override {
        ...
        filteredLeft = 0.0f;
        filteredRight = 0.0f;
        updateFilterCoefficient(cutoffHz.load());
    }
    ...
    void process(float& left, float& right) override {
        ...
        const float coefficient = filterCoefficient.load();
        filteredLeft = delayedLeft + coefficient * (filteredLeft - delayedLeft);
        filteredRight = delayedRight + coefficient * (filteredRight - delayedRight);
        const float wetLeft = filteredLeft;
        const float wetRight = filteredRight;
    ...
    void setCutoffHz(float value) {
        const float cutoff = std::clamp(value, 20.0f,
                                        static_cast<float>(sampleRate * 0.49));
        cutoffHz.store(cutoff);
        updateFilterCoefficient(cutoff);
    }
...
private:
    ...
    void updateFilterCoefficient(float cutoff) {
        filterCoefficient.store(static_cast<float>(
            std::exp(-twoPi * static_cast<double>(cutoff) / sampleRate)));
    }
    ...
    std::atomic<float> filterCoefficient;
};
